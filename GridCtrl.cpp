...
CImageList* CGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
    ...
    *pHotSpot = rect.BottomRight(); 
    ...
    CDC CustomMemDC;
    CBitmap bm;
    CustomMemDC.CreateCompatibleDC(pDC);
    bm.CreateCompatibleBitmap(pDC, rect.Width(), rect.Height());
    CBitmap* pOldBitmap = CustomMemDC.SelectObject(&bm);
    CustomMemDC.SetWindowOrg(0, 0);
    
    // Draw cell onto bitmap in memDC
    CGridCellBase* pCell = GetCell(cell.row, cell.col);
    if (pCell)
        pCell->Draw(&CustomMemDC, cell.row, cell.col, rect, FALSE);
    
    // Clean up
    CustomMemDC.SelectObject(pOldBitmap);
    ReleaseDC(pDC);
